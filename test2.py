...
def model():
    ...
    if request.method == 'POST':
        ...
        # Use the encoder to get the compressed feature representation
        encoded_features = encoder.predict(np.expand_dims(features, axis=0))
        
        # Compare encoded features to the dataset
        dataset_features = dataset.drop(columns=['label']).values
        distances = np.linalg.norm(dataset_features - encoded_features, axis=1)
