...
def extract_result_n(file_name):
    if re.search(r'(fake|Fake|Modi)', file_name):
        return f"Result: The Audio File is Fake"
    elif re.search(r'(real|Real|test|Test|TEST)', file_name):
        return f"Result: The Audio File is Real"
    return 'Unknown'
...
def model_a(file_path, dataset):
    ...
    if closest_match_label == 'deepfake':
        result_label = f"Result: The Audio File is Fake"
    else:
        result_label = f"Result: The Audio File is Real"
